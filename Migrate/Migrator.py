            ...
            for volinfo in self.__migrateOptions.getDataVolumes():
                ...
                if self.__skipUpload == False:
                    ...
                    self.__dataChannelList[volinfo.getVolumePath()] = self.__cloudOptions.generateUploadChannel(self.__dataMediaList[volinfo.getVolumePath()].getMaxSize() , \
                        self.__cloudOptions.getServerName() or description,  volinfo.getUploadPath() , self.__resumeUpload or self.__backupMode , self.__dataMediaList[volinfo.getVolumePath()].getImageSize(), self.__backupMode )
