        ...
        while 1:
            ...
            while retries < self.__maxRetries:
                ...
                try:
                    ...
                    logging.warning("!Failed to upload data: %s/%s , making a retry...", str(bucket), str(offset))
            ...
            if failed:
                logging.error("!!! ERROR failed to upload data: %s/%s!", str(bucket), str(offset))
...
class S3UploadChannel(UploadChannel.UploadChannel):
    ...
    def reconfirm(self, uploadid):
        ...
        for match in re.finditer(r"<byte-range end=\"([0-9]+)\" start=\"([0-9]+)\"\s*/>[^<]*<key>([^<]+)</key>" , xml , re.MULTILINE + re.DOTALL):
